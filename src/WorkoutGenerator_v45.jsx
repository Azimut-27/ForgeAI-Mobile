...
export default function WorkoutGenerator() {
  ...
  const AITab = () => {
    ...
    const sendPrompt = (prompt = aiPrompt) => {
      const cleanPrompt = prompt.trim();
      if (!cleanPrompt) return;
      setAiPrompt(cleanPrompt);
      setAiResponse(`ForgeAI Coach: Here is a premium adjustment for "${cleanPrompt}". Keep your main lift quality high, cap failure work to the final accessory, and use a 2-week progression wave before reassessing readiness.`);
    };
      ...
      <div className="space-y-5">
        ...
        <PremiumCard variant="primary" className="space-y-4">
          ...
          <div className="flex gap-2 rounded-[1.5rem] border border-white/[0.08] bg-black/22 p-2">
            ...
            />
            <button type="button" onClick={() => sendPrompt()} className="grid h-11 w-11 place-items-center rounded-full bg-amber-100 text-zinc-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-200">
        ...
        <PremiumCard variant="secondary">
          ...
          <p className="text-sm leading-7 text-zinc-300">{aiResponse}</p>
        </PremiumCard>
  ...
  const ProTab = () => {
    const features = [
      ['Adaptive Progression', BarChart3],
      ['Recovery Intelligence', Heart],
      ['Advanced Analytics', Gauge],
      ['Exercise Library', ClipboardList],
      ['Periodized Plans', Layers],
      ['Coach Mode', Brain]
    ];
      ...
      <div className="space-y-5">
        <PremiumCard variant="hero">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="mb-2 text-[0.66rem] font-semibold uppercase tracking-[0.24em] text-amber-100/60">Upgrade</div>
              <h1 className="text-5xl font-black tracking-[-0.07em] text-white">ForgeAI Pro</h1>
              <p className="mt-4 text-sm leading-7 text-zinc-400">Advanced programming, deeper analytics, smarter progression.</p>
            </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {features.map(([feature, Icon]) => (
            <PremiumCard key={feature} variant="secondary" className="flex items-center gap-3">
              <IconBubble icon={Icon} />
              <div className="text-base font-bold tracking-[-0.03em] text-white">{feature}</div>
            </PremiumCard>
          ))}
        </div>
        <PremiumCard variant="hero">
          <div className="text-[0.66rem] font-semibold uppercase tracking-[0.24em] text-amber-100/60">Founders Plan</div>
          <div className="mt-2 text-4xl font-black tracking-[-0.06em] text-white">EUR 9.99/mo</div>
          <p className="mt-3 text-sm leading-6 text-zinc-400">Unlock periodized plans, Coach Mode, progression intelligence, and advanced training analytics.</p>
          <button type="button" className="mt-6 w-full rounded-full bg-amber-100 px-6 py-4 font-black text-zinc-950 shadow-[0_18px_70px_rgba(245,158,11,0.22)] transition hover:bg-amber-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-200">
            Unlock Pro
          </button>
        </PremiumCard>
      </div>
